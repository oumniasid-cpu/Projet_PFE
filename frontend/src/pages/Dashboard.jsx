...
const Dashboard = () => {
  ...
  // Each endpoint has its own slice of state for granular loading/error display
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (statsRes.status === 'fulfilled' && statsRes.value) {
        ...
        newErrors.stats = statsRes.reason?.message || 'Erreur stats';
      }

      if (budgetRes.status === 'fulfilled' && budgetRes.value) {
        // /budget-history returns an array directly
        setBudgetHistory(Array.isArray(budgetRes.value) ? budgetRes.value : []);
  ...
  // ── Critical error (stats failed AND no fallback) ────────────────────────────
  if (!stats && errors.stats) return (
    <div className="flex h-screen items-center justify-center bg-gray-50">
      <div className="text-center space-y-3">
        <AlertTriangle className="w-12 h-12 text-red-400 mx-auto" />
        <p className="text-gray-700 font-semibold">Erreur de chargement</p>
        <p className="text-gray-400 text-sm">{errors.stats}</p>
        <button
          onClick={() => window.location.reload()}
          className="flex items-center gap-2 mx-auto px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 transition"
        >
  ...
  const progressData   = stats?.individualProgress || [];   // for the bar chart
  // Prefer the richer /projects list; fallback to recentProjects from /stats
    ...
    <div className="flex h-screen bg-gray-100 text-gray-900">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <main className="flex-1 overflow-y-auto p-6 lg:p-8 space-y-8">
          ...
          {/* Non-critical error banner (one or more endpoints failed but we have fallbacks) */}
          {Object.keys(errors).length > 0 && (
            <div className="flex items-center gap-3 bg-yellow-50 border border-yellow-200 text-yellow-800 text-sm px-4 py-3 rounded-xl">
              ...
              <span>
                Certaines données sont partielles :&nbsp;
              ...
              <div>
                ...
                <div className="w-24 h-1.5 bg-gray-100 rounded-full mt-1.5">
                  <div
                    ...
                    style={{ width: `${Math.min(metrics.avg_progress || 0, 100)}%` }}
                  />
                </div>
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            {/* AreaChart — data from /budget-history (with /stats fallback) */}
                ...
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={budgetHistory}>
                      ...
                      <YAxis axisLine={false} tickLine={false} tick={{ fill: '#9ca3af', fontSize: 12 }} tickFormatter={(v) => formatMoney(v)} />
                      <Tooltip
            ...
            {/* BarChart — data from /stats → individualProgress */}
            <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
              <div className="mb-6">
                <h3 className="font-semibold text-gray-800">Avancement par Projet</h3>
                <p className="text-sm text-gray-400 mt-0.5">Top 4 projets récents</p>
              </div>
          ...
          {/* ── Recent Projects Table — data from /projects ───────────────────── */}
